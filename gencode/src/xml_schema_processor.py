...
def process_composites(element: Element, xpath: str, namespaces: dict[str, str], version: str, defined_types: dict[str, str], target_src_code_directory: str, area_name: str, service_name: str = None) -> int:
  warning_counter = 0
  ...
  for composite in composites:
    ...
    if class_name not in defined_types:
      class_code = code_generator.generate_class_code(class_name, composite, namespaces, version, defined_types, target_src_code_directory)
      ...
      defined_types[class_name] = utils.to_snake_case(class_name)
...
def process_enums(element, xpath: str, namespaces: dict[str, str], version, defined_types: dict[str, str], target_src_code_directory: str, area_name: str, service_name: str = None):
  ...
  for enum in enums:
    ...
    class_code = code_generator.generate_enum_class_code(class_name, enum, namespaces, version)
...
def process_enums_errors(element: Element, xpath: str, namespaces: dict[str, str], version: str, target_src_code_directory: str, area_name: str, service_name: str = None):
  enums = element.findall(xpath, namespaces=namespaces)
  errors = []
  for enum in enums:
    errors.append((enum.get('number'), enum.get('name'), enum.get('comment')))

  class_name = f"{area_name.upper()}Errors"
  class_code = code_generator.generate_enum_errors_class_code(class_name, errors, version)
...
def process_xml_content(xml_content, defined_types: dict[str, str], target_src_code_directory: str):
  ...
  namespaces = utils.get_namespaces(xml_content)
  ...
  process_enums_errors(
    element=root,
    xpath='./mal:area/mal:errors/mal:error',
...
def write_generated_classes(generated_classes, target_directory):
  for class_name, class_code in generated_classes:
    file_name = utils.to_snake_case(class_name) + ".py"
...
def correct_import_statement(target_src_code_directory, module_name, class_name):
  class_file = utils.find_class_file(target_src_code_directory, class_name)
  if class_file:
